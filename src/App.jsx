...
// 2D ultrafiltration simulation with vertical membrane
// - Feed flows from left pipe into pressurised chamber
// - Pressure acts in the same direction as flow (left ➝ right)
// - Vertical membrane separates feed/retentate (left) and permeate (right)
// - Solute particles move under pressure; if size < pore size → they pass
// - Membrane health decreases with use (fouling concept), can be partially restored by backwash

const VESSEL_WIDTH = 680;
const VESSEL_HEIGHT = 420;

// Vessel geometry
const VESSEL_X = 120;
const VESSEL_Y = 30;
const VESSEL_W = VESSEL_WIDTH - 180; // main chamber width
const VESSEL_H = VESSEL_HEIGHT - 80; // main chamber height
const MEMBRANE_X = VESSEL_X + VESSEL_W * 0.55; // vertical membrane position

function clamp(v, min, max) {
  return Math.max(min, Math.min(max, v));
}

function createParticles(concentrationValue, meanSize) {
  ...
  for (let i = 0; i < particleNumber; i++) {
    ...
    particles.push({
      ...
      vx: 0,
      vy: 0,
      region: "feed", // "feed" | "retentate" | "permeate"
    });
  }
  return particles;
}
...
export default function UltraFiltrationSim() {
  ...
  // For keyboard control of sliders (Tab) and play/pause (Space)
...
function StatCard({ label, value, unit, description }) {
  return (
    <div className="bg-slate-900/80 rounded-2xl border border-slate-800 px-4 py-3 flex flex-col gap-1 shadow-md shadow-black/30">
